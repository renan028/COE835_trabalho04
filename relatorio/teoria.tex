...
%
O diagrama de blocos da implementação de controle 2DOF pode ser visto na \fref{fig:blocks} e sua forma em estrutura 2DOF é mostrada na figura \fref{fig:blocks2}.
%
\begin{figure}[H]
  \centering
...
\end{equation}
%
em que utilizamos a forma canônica controlável para a realização de estados:
%
\begin{equation}
 A_f = \mat{
 0 & 1 & 0 & \cdots & 0 & 0 \\
...
solução, igualando o numerador, são:
%
\begin{align}
...
fator de $(\Lambda - F)$, isto é, $(\Lambda - F) = NH$, em que $\textrm{grau}(F) = n-2$, $\textrm{grau}(N) = m$ e
$\textrm{grau}(H) = n-m-1 = n^*-1$. Após o cancelamento de $N$, deve-se
resolver a \textit{Equação Diophantina} para os polinômios desconhecidos $H$ e $G$:
%
\begin{equation}
HD - k_pG = D_mA_0 \,, 
\end{equation}
...
$\textrm{grau}(D_mA_0)=2n-m-1$ e $H(s)$ é mônico. E, por hipótese,
$\textrm{grau}(D)-\textrm{grau}(N)=\textrm{grau}(D_m)-\textrm{grau}(N_m)=n^*$
(grau relativo invariante entre planta e modelo de referência).

Após encontrar os polinômios $G(s)$ e $H(s)$, obtém-se os parâmetros $\theta^*_n$ e $\theta^*_2$ usando \eqref{eq:theta_n} e \eqref{eq:theta_2}, e o polinômio $F(s)$ através de:
%
\begin{equation}
 F(s) = \Lambda(s) - N(s)H(s) \,.
\end{equation}

Finalmente, é obtido o vetor $\theta^*_1$ usando a equação \eqref{eq:theta_1}.
